...
export default function TopBar({ activeNav, role, onLogout }) {
  const labels = {
    dashboard: "Dashboard",
    ...
    "validacion-fuentes": "Validación de Fuentes",
    ...
    <View style={{ backgroundColor: C.card, borderBottomWidth: 1, borderBottomColor: C.border, paddingHorizontal: 24, paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
      <View>
        <Row style={{ alignItems: "center", gap: 5 }}>
          ...
          <Text style={{ fontSize: 11, color: C.textMuted }}>{labels[activeNav]}</Text>
        </Row>
        <Text style={{ fontSize: 17, fontWeight: "800", color: C.text, marginTop: 1 }}>{labels[activeNav]}</Text>
      </View>
      <Row style={{ alignItems: "center", gap: 12 }}>
        ...
        <View style={{ width: 36, height: 36, borderRadius: 9, backgroundColor: C.bg, borderWidth: 1, borderColor: C.border, alignItems: "center", justifyContent: "center" }}>
